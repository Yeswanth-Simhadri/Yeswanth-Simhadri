<h1 align="center">🚀 Yeswanth Simhadri</h1>

<p align="center">
  🎓 B.Tech (CSE – AI & ML) @ GMR Institute of Technology | 📈 CGPA: 9.30 <br/>
  ☁ AWS Cloud Intern @ HashTek | 🧠 200+ LeetCode Problems Solved | 👨‍💻 Full-Stack & AI Enthusiast
</p>

---

### 🛠 Tech Stack

- 💻 *Languages:* Python | Java | C | JavaScript  
- 🌐 *Web Development:* React | Node.js | Express.js | Bootstrap | HTML | CSS  
- 🧰 *Tools & Platforms:* Git | GitHub  | Power BI | ServiceNow (Admin & App Dev)  
- ☁ *Cloud:* AWS (EC2, S3, IAM, VPN, Load Balancers)  
- 🧪 *Machine Learning & Data:* scikit-learn | pandas | numpy | matplotlib | Streamlit  
- 🗃 *Databases:* MongoDB | MySQL

---

### 💼 Featured Projects

| 📌 Project | 📝 Description |
|-----------|----------------|
| 🏠 *Wanderlust* | Airbnb-like listing app with authentication, CRUD operations, Cloudinary image uploads, and review system. |
| 💸 *AIconomy* | Personal finance tracker with dashboards, bill scanner (Gemini Flash), and budget alerts. |
| 🎬 *Movie Recommendation System* | Streamlit-based ML app recommending movies using content & collaborative filtering. |

> 👉 Explore my [GitHub Projects](https://github.com/Yeswanth-Simhadri?tab=repositories) to see code & demos!

---

### 📜 Certifications

- 🧾 **ServiceNow CSA & CAD** – Global Certification (May 2025)  
- 🎓 **DSA using Java** – NPTEL, IIT Kharagpur (Oct 2024)  
- 🤖 **Prompt Engineering for ChatGPT** – Coursera, Vanderbilt University (Feb 2025)  
- 📊 **Data Analytics Basics** – IBM (Jul 2024)

---

### 🏆 Achievements

- ✅ **LeetCode:** Solved 200+ coding problems  
- 🧑‍💼 **Leadership:** Led mini project team to successful completion  
- 🎤 **Event Coordination:** Organized Paper Presentation @ STEPCONE 2025 (50+ participants)

---

### 🌐 Connect with Me

- 🔗 [LinkedIn](https://www.linkedin.com/in/yeswanth-simhadri-743699289/)  
- 💌 *Email:* yeswanth1704@gmail.com
-  🌍 [Portifolio](https://yeswanthsimhadri.vercel.app/)
- 💻 [GitHub](https://github.com/Yeswanth-Simhadri)  
- 🧠 [LeetCode](https://leetcode.com/u/yeswanth1704/)  
- 🌍 [Wanderlust Project](https://github.com/Yeswanth-Simhadri/wanderlust-project)  
- 🌍 [AIconomy Project](https://github.com/Yeswanth-Simhadri/AIconomy)


---

### 🎯 Fun & Goals

- 🧩 Love solving logic puzzles & DSA challenges  
- 💡 Passionate about building AI-based, user-focused apps  
- 🌟 Always exploring new tech & frameworks  

---
